...
{{#to_Entity}}
  ...
  entity {{EntityTechnicalName}} as projection on my.{{EntityTechnicalName}} {{#Hasto_Action}} actions {
    {{#to_Action}}
    action {{ActionTechnicalName}}({{#to_ActionParameter}}{{#ActionParameterLabel}}@(title:'{i18n>{{ActionTechnicalName}}-{{ActionParameterTechnicalName}}}'){{/ActionParameterLabel}}{{ActionParameterTechnicalName}} : {{FieldType}}{{^isLast}},{{/isLast}}{{/to_ActionParameter}});
    {{/to_Action}}
