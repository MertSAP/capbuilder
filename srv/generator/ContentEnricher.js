module.exports = class ContentEnricher {
  ...
  setServiceEntities() {
    for (const entityRow of this.templateData.Services[0].to_Entity) {
      ...
      if (
        entityRow.EntityParentRelationships.length === 0 &&
  ...
  enrichValueHelp() {
    for (const entityRow of this.templateData.Services[0].to_Entity) {
      for (const valueHelpRow of entityRow.to_ValueHelp) {
        const vhEntity = this.templateData.Services[0].to_Entity.find(
          (item) => {
            return (
              item.EntityTechnicalName ===
              valueHelpRow.ValueHelpEntity.EntityTechnicalName
            );
          }
        );
  ...
  enhanceListAndObjectPage() {
    if (this.templateData.Services[0].ObjectPages.length > 0) {
      this.templateData.Services[0].ListPages[
        this.templateData.Services[0].ListPages.length - 1
      ].isLast = false;
    }
  }
  ...
  */
  isRoot(EntityTechnicalName) {
    return this.rootEntityTechnicalName === EntityTechnicalName;
  }
  ...
  */
  findPath(entity, parent) {
    if (parent !== undefined) {
      ...
      if (this.isCircular(entity.PathToRoot, parent.EntityTechnicalName)) {
        const outputRelationship = this.generatePathToRootString(
          entity.PathToRoot
        );
        throw new Error(
      ...
      entity.PathToRoot.push({
        EntityTechnicalName: parent.EntityTechnicalName,
      });
    }
  ...
  generateManifestData() {
    ...
    for (const entityRow of this.templateData.Services[0].to_Entity) {
      ...
      if (this.isRoot(entityRow.EntityTechnicalName)) {
        this.templateData.Services[0].ListPages.push(
          this.generateListEntry(entityRow)
        );
        entityRow.EntityIsRoot = true;
      }
      this.templateData.Services[0].ObjectPages.push(
        this.generateObjectPageEntry(entityRow)
      );
    }
  }
  ...
  generateEntityPath(EntityTechnicalName, level) {
    if (level === 0) {
      return (
        EntityTechnicalName +
        "({" +
        this.getEntityKeyNumber(EntityTechnicalName) +
        "})"
      );
    } else {
      return (
  ...
  getEntityKeyNumber(EntityTechnicalName) {
    let index = this.templateData.Services[0].to_Entity.findIndex((item) => {
      return item.EntityTechnicalName === EntityTechnicalName;
    });

    if (
      this.isRoot(
        this.templateData.Services[0].to_Entity[index].EntityTechnicalName
      )
    ) {
      return "key";
    }
  ...
  addObjectPageNavigation(entity, manifestFields) {
    ...
    for (const childRow of entity.EntityChildRelationships) {
      ...
      if (childEntity.RequriesObjectPage) {
        manifestFields.navigation.push({
          assoication: "to_" + childEntity.EntityTechnicalName,
          route: childEntity.EntityTechnicalName + "ObjectPage",
        });
  ...
  generatePattern(entity) {
    if (this.isRoot(entity.EntityTechnicalName)) {
      return (
        this.generateEntityPath(entity.EntityTechnicalName, 0) + ":?query:"
      );
    }
    let level = 0;
    let path = "";
    ...
    for (const ancestor of entity.PathToRoot) {
      path =
        path + this.generateEntityPath(ancestor.EntityTechnicalName, level);
      level++;
    }
  ...
  enrichRoles() {
    ...
    for (const roleRow of this.templateData.Services[0].to_ServiceRole) {
      for (const roleAuthRow of roleRow.to_ServiceAuth) {
        if (
          authMap.get(roleAuthRow.AuthEntity.EntityTechnicalName) === undefined
        ) {
        ...
        for (const fieldRow of entityRow.to_Field) {
          ...
          if (fieldRow.Facet !== null) {
            const facetRow = entityRow.to_Facet.find((item) => {
              return (
                item.FacetTechnicalName === fieldRow.Facet.FacetTechnicalName
              );
            });
