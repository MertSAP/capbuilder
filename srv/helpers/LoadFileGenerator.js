...
const { Service, Field } = cds.entities("mert.capbuilder");
module.exports = class LoadFileGenerator {
  ...
  async enhanceValueHelps(serviceTree) {
    for (const entityRow of serviceTree.to_Entity) {
      for (const valueHelpRow of entityRow.to_ValueHelp) {
        const entityData = await SELECT.from(Field)
          .where({
            to_Entity_EntityUUID: valueHelpRow.ValueHelpEntity.EntityUUID,
            FieldisKey: true,
          })
          .columns("FieldTechnicalName");
        delete valueHelpRow.ValueHelpEntity.EntityUUID;
        if (entityData.length > 0) {
          valueHelpRow.ValueHelpEntity.KeyField =
            entityData[0].FieldTechnicalName;
        }
      }
    }
    return serviceTree;
  ...
  */
  entityCompare() {
    return function (a, b) {
      ...
      if (!a.isRoot) {
        aSortValue =
          a.EntityParentRelationships.length + a.EntityChildRelationships + 1;
      }
      if (!b.isRoot) {
        bSortValue =
          b.EntityParentRelationships.length + b.EntityChildRelationships + 1;
      }
  ...
  */
  async assignAssociations(serviceTree) {
    ...
    serviceTree.Services[0] = await tracer.traceAndValidateRelationships(
      serviceTree.Services[0]
    );
