...
entity Service : managed {
    key ServiceUUID          : UUID;
        ...
        to_Entity            : Composition of many Entity
                                   on to_Entity.to_Service = $self;
        to_Association       : Composition of many Association
                                   on to_Association.to_Service = $self;
        to_ServiceRole       : Composition of many ServiceRole
                                   on to_ServiceRole.to_Service = $self;
}
...
entity Entity : managed {
    key EntityUUID               : UUID;
        ...
        EntitySuggestedValues    : String;
        EntityIsRoot             : Boolean;
        to_Service               : Association to Service;
        to_Field                 : Composition of many Field
                                       on to_Field.to_Entity = $self;
        to_Action                : Composition of many Action
                                       on to_Action.to_Entity = $self;
        to_Facet                 : Composition of many Facet
                                       on to_Facet.to_Entity = $self;
        to_ValueHelp             : Composition of many ValueHelp
                                       on to_ValueHelp.to_Entity = $self;
}
...
entity Action : managed {
    key ActionUUID          : UUID;
        ...
        to_ActionParameter  : Composition of many ActionParameter
                                  on to_ActionParameter.to_Action = $self;
}
...
entity Field : managed {
    key FieldUUID             : UUID;
        ...
        FieldLabel            : String(50)               @mandatory;
...
entity ServiceRole : managed {
    key RoleUUID          : UUID;
        ...
        to_ServiceAuth    : Composition of many ServiceAuth
                                on to_ServiceAuth.to_ServiceRole = $self;
        to_Service        : Association to Service;
}
